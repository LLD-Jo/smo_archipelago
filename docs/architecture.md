...
The forked apworld registers as `Manual_SMO_archipelago` (vs. the Manual client's `Manual_SMO_mp3`). The client's `Connect` packet uses this exact game name. Seeds generated by either world are not interchangeable; that's intentional — the real-client seeds gain richer enforcement options the Manual client cannot honor.
