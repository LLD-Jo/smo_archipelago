...
| **Python 3.12** | The moon/capture extractor (`oead` has no Python 3.13+ wheel) | https://www.python.org/downloads/release/python-3120/ |
...
| **A modded Switch** OR **Ryujinx** | Where SMO actually runs | Atmosphere CFW on a modded Switch, or [Ryujinx](https://ryujinx.org/) |
...
3. **Generate a multiworld** with at least one SMO slot. You'll get a
   per-player zip back containing a `<player>.smoap` file alongside the
   usual AP outputs.
4. **Double-click your `.smoap` file.** Archipelago Launcher routes it to
   SMO Client, which notices you haven't set up yet and opens the setup
   wizard.
5. **Walk the wizard.** Eight pages, in order:
   ...
   7. Deploy target — pick **Real Switch (SD card)** or **Ryujinx**:
      - **Ryujinx:** wizard copies to `%APPDATA%/Ryujinx/mods/contents/...`
        (same path Ryujinx itself expects). Just boot SMO in Ryujinx
        afterwards.
...
6. **Boot SMO.** On the Switch or in Ryujinx — the mod loads on game start.
...
https://www.python.org/downloads/release/python-3120/ alongside any other
Pythons.
